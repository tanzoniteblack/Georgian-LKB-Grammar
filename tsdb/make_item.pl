...
sub parse_example {
	...
	if (&id_used($i_id)) {
	    die_verbose "item id $i_id is not unique.\n";
	}
	$line = &next_line();
    }
    ...
    ($line =~ /^[Ss]ource:\s*(.*\S*)\s*/) or die_verbose "Expected line to start with \"Source\".  Actual line: $line\n";
    ...
    $line = &next_line();
    ($line =~ /^[Vv]etted/) or die_verbose "Expected line to start with \"Vetted\".  Actual line: $line\n";


    ### Judgment
    $line = &next_line();
    ($line =~ /^[Jj]udgment:\s*(.*\S*)\s*/) or die_verbose "Expected line to start with \"Judgment\".  Actual line: $line\n";

    # input should be U or G, but output 0 or 1, respectively
    $judgment = $1;
    ($judgment =~ /^[UuGg]$/) or die_verbose "Bad value for Judgment, should be U or u or G or g: $judgment\n";
    $i_wf = ($judgment =~ /[Uu]/)? 0 : 1;


    ### Phenomena
    $line = &next_line();
    ($line =~ /^[Pp]henom.*:\s*(.*\S*)\s*/) or die_verbose "Expected line to start with \"Phenom\".  Actual line: $line\n";
    $i_category = $1;
